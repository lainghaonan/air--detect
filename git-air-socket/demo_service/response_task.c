...
#define DESC_SIZE   64      //服务简介长度
#define ADDR_SIZE   16      //地址长度
#define USAGE_SIZE  256     //用法长度
...
static void Query_Handler(UdpPoint* udp, Message* msg, const char* remote, int port)
{
    ...
    if( resp )
    {
        strncpy(resp->payload, Service_GetDesc(), DESC_SIZE);           //描述设备可提供服务
        strncpy(resp->payload + DESC_SIZE, Wifi_IpAddr(), ADDR_SIZE);   //设备IP地址
        strncpy(resp->payload + DESC_SIZE + ADDR_SIZE, Service_GetUsage(), USAGE_SIZE);//设备用法

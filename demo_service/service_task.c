...
static char* FormatNumber(float n)  //将float格式化字符串
{
    char* ret = malloc(16);
...
static const int g_size = sizeof(g_handler)/sizeof(*g_handler);     //g_handler数组元素个数
...
static void EventListener(TcpClient* client, int evt)
{
    ...
    else if( evt == EVT_DATA )
    {
        ...
        if( msg && (msg->type == TYPE_TOUCH) )  //tcp-client发送的是touch命令
        {
            printf("%s : msg = %p\n", __FUNCTION__, msg);
            ...
            char* resp = TouchService((char*)msg->payload); //根据touch命令执行服务

            free(msg);

            //根据不同的情况，回复tcp—client不同的消息
            if( resp )
            {
